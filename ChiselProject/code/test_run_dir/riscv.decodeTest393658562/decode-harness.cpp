...
#endif
#include <iostream>
class decode_api_t: public sim_api_t<VerilatorDataWrapper*> {
public:
    decode_api_t(Vdecode* _dut) {
        dut = _dut;
        main_time = 0L;
        is_exit = false;
#if VM_TRACE
    ...
    void init_sim_data() {
        ...
        sim_data.inputs.push_back(new VerilatorCData(&(dut->io_xs2)));
        sim_data.inputs.push_back(new VerilatorCData(&(dut->io_xs1)));
        sim_data.inputs.push_back(new VerilatorCData(&(dut->io_xd)));
        sim_data.inputs.push_back(new VerilatorCData(&(dut->io_func)));
        sim_data.outputs.push_back(new VerilatorCData(&(dut->io_ctrl_out)));
        sim_data.signals.push_back(new VerilatorCData(&(dut->reset)));
        sim_data.signal_map["decode.reset"] = 0;
    }
#if VM_TRACE
...
static decode_api_t * _Top_api;
double sc_time_stamp () { return _Top_api->get_time_stamp(); }
int main(int argc, char **argv, char **env) {
    Verilated::commandArgs(argc, argv);
    Vdecode* top = new Vdecode;
    std::string vcdfile = "test_run_dir/riscv.decodeTest393658562/decode.vcd";
